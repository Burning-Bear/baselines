...
def traj_segment_generator(pi, env, horizon, stochastic, time_step_holder):
    """
    this function is to generator a trajectory segment. 
    ...
    horizon : timestep per episode

    stochastic : if use stochastic policy or not.
    ...
    while True:
        ...
        acs[i] = ac
        prevacs[i] = prevac
        # new: if terminal or not.
        ob, rew, new, _ = env.step(ac)
        episode_timestep += 1
        if episode_timestep > 0 and episode_timestep % 2000 == 0:
            new = True
        rews[i] = rew
...
def add_vtarg_and_adv(seg, gamma, lam):
    """
    calculate value target and advantage function.
    ...
    # vpred: value predict
    # nextvpred: the last predict value. size(seg["nextvpred"] == 1 )
...
def learn(env, policy_fn, *,
    ...
    # ----------------------------------------
    seg_gen = traj_segment_generator(pi, env, timesteps_per_batch, stochastic=True, time_step_holder=tester.time_step_holder)
